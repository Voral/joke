...
2. Добавляет в валидных ответах заголовок X-Csrf-Token со значением токена
3. Для небезопасных методов (POST, PUT, DELETE и др.) проверяет наличие токена:
    - в параметрах запроса: ?csrf_token=... или csrf_token=... (POST),
    - в заголовке: X-Csrf-Token: ....
4. При несоответствии выбрасывает CsrfMismatchException (HTTP 403).

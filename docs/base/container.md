...
> ⚠️ `make()` **не кэширует** результат. Каждый вызов создаёт новый объект. Для получения синглтонов используйте `get()`.
...
**`ParameterResolver`** (`Vasoft\Joke\Container\ParameterResolver`) для анализа параметров. Процесс состоит из двух этапов:
...
- **Рефлексия** используется при каждом создании прототипа и при каждом вызове `make()` (кэширование метаданных рефлексии планируется в будущих версиях).
